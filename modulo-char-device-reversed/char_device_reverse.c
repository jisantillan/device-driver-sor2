...
#define DEVICE_NAME "chardevicereverse"
...
void reverse_mensaje(char * buffer) {
    int longitud;
    int i, j;
    char temp;
    if (buffer == NULL) {
        return; // No hacer nada si el mensaje es nulo
    }
    if (strlen(mensaje) <= 0)
    {
        return;
    }
    longitud = strlen(mensaje) - 2; // Se determina esta longitud para evitar el problema de salto de linea en el ultimo caracter.
    for (i = 0, j = longitud; i < j; i++, j--) {
        // Comienza asignando el ultimo caracter del buffer en el primero, incrementando la pos i y decrementando j
        temp = buffer[i];
        buffer[i] = buffer[j];
        buffer[j] = temp;
    }
...
MODULE_DESCRIPTION("Modulo que devuelve lo último que fue escrito al reves");
